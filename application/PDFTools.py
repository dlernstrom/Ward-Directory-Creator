...
from reportlab.platypus import Paragraph, Table, TableStyle, Image, Frame, Spacer, Preformatted, PageBreak, ParagraphAndImage
...
class PDFTools:
    ...
    def GenerateWardPagination(self):
        ...
        while self.FlowablesConsumed < self.TotalFlowables:
            ...
            if Header_Footer:
                MyFrame.add(self.ChurchFlowable,
                            pdf_TEST)
                MyFrame.add(Paragraph('Page ' + str(pdf_TEST.getPageNumber() - 1),
                                      self.styles['DaveHeaderLeft']),
                            pdf_TEST)
            try:
                PageFull = False
                while not PageFull:
                ...
                print "%s: %s" % (str(type(data)), str(data))
            ...
            FlowablesOnPages.append([pdf_TEST.getPageNumber() - 1,
                                     StartingFlowablesConsumed,
                                     NumberIConsumed,
                                     NumberRemaining])
            print FlowablesOnPages[-1]
    ...
    def TableizeFamily(self, household):
        Family = []
        if self.DEBUG:
            print household.surname,'Family'
        ...
        for member in household.family:
            ...
            if member.isParent:
                Column = 1
                cellStyle = self.styles['DaveBoldSmall']
            else:
                Column = 2
                cellStyle = self.styles['DaveHeading']
